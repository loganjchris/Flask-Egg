...
def hello():
    ...
    if request.method == 'POST':
        if session['room_number'] == 1818:
            flash(r"The first part of the code is eggFlag{who-made-this}")
        else:
...
def scoreboard():
    ...
    flags = {0:r"eggFlag{extra-points-before-breakfast}", 1:r"eggFlag{who-made-this}", 2:r"eggFlag{view-source}"}
    
    if 'challenges' not in session:
        session['challenges'] = 0
        session.modified = True
    if 'flags' not in session:
        session['flags'] = {0 : False, 1 : False, 2 : False}
